...
if (HAS_NOSTR_TOOLS) {
  ...
  const { validatePrivateKey, isValidPubkey, normalizePubkey } = await import("./nostr-bus.js");
  const { createSeenTracker } = await import("./seen-tracker.js");
  ...
  describe.skip("nostr-bus fuzz (requires nostr-tools)", () => {});
}
