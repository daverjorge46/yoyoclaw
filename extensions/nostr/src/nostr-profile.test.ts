...
import type { ProfileContent } from "./nostr-profile.js";
...
if (HAS_NOSTR_TOOLS) {
  const { verifyEvent, getPublicKey } = await import("nostr-tools");
  const {
    createProfileEvent,
    profileToContent,
    contentToProfile,
  ...
  describe.skip("nostr-profile (requires nostr-tools)", () => {});
}
