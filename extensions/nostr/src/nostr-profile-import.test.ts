...
const HAS_NOSTR_TOOLS = (() => {
  try {
    _require.resolve("nostr-tools");
    return true;
  } catch {
    return false;
  }
...
if (HAS_NOSTR_TOOLS) {
  // type-only import is safe (erased at runtime)
  const { mergeProfiles } = await import("./nostr-profile-import.js");
  type NostrProfile = import("./config-schema.js").NostrProfile;
  ...
  describe.skip("nostr-profile-import (requires nostr-tools)", () => {});
}
