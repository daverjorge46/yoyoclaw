...
/**
 * Load a JSON file that may be encrypted.
 * For backwards compatibility, returns undefined on missing file.
 * THROWS on decryption failure to prevent silent credential loss.
 */
export function loadSecureJsonFile(pathname: string): unknown {
  ...
  switch (result.status) {
    ...
    case "success":
      return result.data;
    case "error":
      // Throw instead of silently returning undefined to prevent credential loss
      throw new Error(`crypto-store: ${result.reason}`);
  }
}
