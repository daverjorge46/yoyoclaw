...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "yoyoclaw.mjs"), "export {};\n", "utf-8");
  });
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "yoyoclaw.mjs")} doctor --non-interactive`]: {
        stdout: "",
      },
    });
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "openclaw");
    const staleDir = path.join(nodeModules, ".yoyoclaw-stale");
  ...
  it("fails with a clear reason when yoyoclaw.mjs is missing", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "openclaw", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
    ...
    await fs.rm(path.join(tempDir, "yoyoclaw.mjs"), { force: true });
  ...
  it("repairs UI assets when doctor run removes control-ui files", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (
        key === `${process.execPath} ${path.join(tempDir, "yoyoclaw.mjs")} doctor --non-interactive`
      ) {
