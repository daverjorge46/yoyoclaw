...
export async function handleCommands(params: HandleCommandsParams): Promise<CommandHandlerResult> {
  ...
  if (resetRequested && params.command.isAuthorizedSender) {
    ...
    const fallbackKey = params.sessionKey ? null : resolveHookFallbackKey(params.ctx);
    const hookSessionKey = params.sessionKey || fallbackKey || "command:unknown";
    let hookMessages: string[] = [];
    const clonedSessionEntry = params.sessionEntry ? safeClone(params.sessionEntry) : undefined;
    const clonedPreviousEntry = params.previousSessionEntry
      ? safeClone(params.previousSessionEntry)
      : undefined;
    try {
