...
export async function runPreparedReply(
  ...
  const additionalSystemPrompt = cfg.agents?.defaults?.additionalSystemPrompt?.trim() ?? "";
  const extraSystemPrompt = [groupIntro, groupSystemPrompt, additionalSystemPrompt]
    .filter(Boolean)
    .join("\n\n");
