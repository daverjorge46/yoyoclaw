...
const EXTRA_MARKERS = ["yoyoclaw", "openclaw", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["openclaw_service_marker", "yoyoclaw_service_marker"];
  const kindKeys = ["openclaw_service_kind", "yoyoclaw_service_kind"];
...
function isOpenClawGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.yoyoclaw.") || label.startsWith("ai.openclaw.");
}
...
function isOpenClawGatewaySystemdService(name: string, contents: string): boolean {
  ...
  if (!name.startsWith("yoyoclaw-gateway") && !name.startsWith("openclaw-gateway")) {
    return false;
  }
...
function isOpenClawGatewayTaskName(name: string): boolean {
  ...
  return (
    ...
    normalized.startsWith("yoyoclaw gateway") ||
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (
      (marker === "yoyoclaw" || marker === "openclaw") &&
    ...
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: (marker !== "yoyoclaw" && marker !== "openclaw") || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "yoyoclaw" && marker !== "openclaw",
    });
  }
