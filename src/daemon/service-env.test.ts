...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENCLAW_SERVICE_MARKER).toBe("yoyoclaw");
    expect(env.OPENCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENCLAW_SERVICE_VERSION).toBe("string");
    expect(env.OPENCLAW_SYSTEMD_UNIT).toBe("yoyoclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENCLAW_LAUNCHD_LABEL).toBe("ai.yoyoclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    ...
    expect(env.OPENCLAW_SYSTEMD_UNIT).toBe("yoyoclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENCLAW_LAUNCHD_LABEL).toBe("ai.yoyoclaw.work");
    }
  });
});
