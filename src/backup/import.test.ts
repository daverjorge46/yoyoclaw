...
async function setupImport(targetStateDir: string, cronStorePath?: string) {
  ...
  vi.doMock("../config/paths.js", () => ({
    resolveConfigPathCandidate: () => path.join(targetStateDir, "openclaw.json"),
    resolveStateDir: () => targetStateDir,
  }));
  if (cronStorePath) {
...
describe("backup/import", () => {
  describe("full import cycle", () => {
    it("imports restoring all files to target state dir", async () => {
        ...
        files: [
          {
            ...
            content: JSON.stringify({ models: { primary: "anthropic/claude-4" } }),
          },
          { archivePath: "workspace/SOUL.md", content: "# Soul\nI am helpful." },
          { archivePath: "workspace/MEMORY.md", content: "# Memory\nDark mode." },
      ...
      const result = await importBackup({ input: archivePath });
    ...
    it("creates .pre-restore.bak of existing config before overwriting", async () => {
      ...
      const targetStateDir = path.join(targetBase, ".openclaw");
      await fs.mkdir(targetStateDir, { recursive: true });
      const existingConfig = JSON.stringify({ existing: true, important: "data" });
      await fs.writeFile(path.join(targetStateDir, "openclaw.json"), existingConfig);

      const { importBackup } = await setupImport(targetStateDir);
      await importBackup({ input: archivePath });
      ...
      expect(JSON.parse(bakContent)).toEqual({ existing: true, important: "data" });
  ...
  describe("dry-run mode", () => {
    it("reports what would be restored without modifying files", async () => {
      ...
      const result = await importBackup({ input: archivePath, dryRun: true });
  ...
  describe("manifest validation", () => {
    it("throws on missing manifest.json", async () => {
      ...
      await expect(importBackup({ input: archivePath })).rejects.toThrow(
        "missing or has an invalid manifest.json",
      );
    });
    ...
    it("throws on invalid manifest version", async () => {
      ...
      await expect(importBackup({ input: archivePath })).rejects.toThrow("Invalid manifest");
    });
  });
  ...
  describe("cron merge", () => {
    it("merges cron jobs when --merge option is used", async () => {
        ...
        files: [
          {
            ...
            content: JSON.stringify({
              version: 1,
              jobs: [
                { id: "job-from-backup", name: "Daily Report", schedule: "0 9 * * *" },
                { id: "shared-job", name: "Backup Updated", schedule: "0 0 * * 0" },
              ],
            }),
          },
        ],
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          jobs: [
            ...
            { id: "shared-job", name: "Original Version", schedule: "0 0 * * 0" },
          ],
        }),
      );
      ...
      const result = await importBackup({ input: archivePath, merge: true });
  ...
  describe("storage backend import", () => {
    it("reads archive from a storage backend", async () => {
      ...
      const result = await importBackup({ input: "stored-backup.tar.gz" }, storage);
  ...
  describe("cron merge edge cases", () => {
    ...
    it("non-merge cron import replaces existing store", async () => {
      ...
      await fs.writeFile(
        cronStorePath,
        JSON.stringify({ version: 1, jobs: [{ id: "old-job", schedule: "0 0 * * *" }] }),
      );
  ...
  describe("manifest edge cases", () => {
    it("throws on manifest with missing required fields", async () => {
      ...
      await fs.writeFile(
        path.join(staging, "manifest.json"),
        JSON.stringify({ version: 1, createdAt: "2026-01-01", openclawVersion: "2026.2.9" }),
      );
  ...
  describe("large archive", () => {
    it("handles archive with many files across components", async () => {
      ...
      files.push({ archivePath: "workspace/SOUL.md", content: "# Large backup soul" });
