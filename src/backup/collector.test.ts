...
import type { BackupComponent, CollectedFile } from "./types.js";
...
describe("backup/collector", () => {
  it("collects config with secrets redacted", async () => {
    ...
    vi.doMock("../config/paths.js", () => ({
      resolveConfigPathCandidate: () => configPath,
      resolveStateDir: () => stateDir,
    }));

    const { collectFiles } = await import("./collector.js");

    const files = await collectFiles({ components: ["config"], stateDir });
  ...
  it("collects workspace files", async () => {
    ...
    vi.doMock("../config/paths.js", () => ({
      resolveConfigPathCandidate: () => path.join(stateDir, "openclaw.json"),
      resolveStateDir: () => stateDir,
    }));

    const { collectFiles } = await import("./collector.js");

    const files = await collectFiles({ components: ["workspace"], stateDir });
  ...
  it("collects sessions", async () => {
    ...
    const files = await collectFiles({ components: ["sessions"], stateDir });
  ...
  it("returns empty array when state dir does not exist", async () => {
    ...
    vi.doMock("../config/paths.js", () => ({
      resolveConfigPathCandidate: () => path.join(nonexistent, "openclaw.json"),
      resolveStateDir: () => nonexistent,
    }));
    // Mock DEFAULT_CRON_STORE_PATH so it doesn't pick up real user data
    vi.doMock("../cron/store.js", () => ({
      DEFAULT_CRON_STORE_PATH: path.join(nonexistent, "cron", "jobs.json"),
    }));

    const { collectFiles } = await import("./collector.js");

    const files = await collectFiles({ stateDir: nonexistent });
...
describe("stripSecrets", () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        ...
        providers: [
          ...
          { name: "openai", apiKey: "sk-openai-secret", nested: { secret: "deep" } },
        ],
      }),
    );
  ...
  it("preserves non-string values even for sensitive key names", async () => {
    ...
    await fs.writeFile(
      path.join(stateDir, "openclaw.json"),
      JSON.stringify({
        features: { token: true, password: 42, apiKey: null, secret: ["array"] },
      }),
    );
