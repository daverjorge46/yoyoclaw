...
async function buildStateDir(baseDir: string): Promise<string> {
  ...
  // Skills
  await fs.mkdir(path.join(stateDir, "skills", "greeting"), { recursive: true });
  await fs.writeFile(
...
describe("backup/export", () => {
  it("exports a valid tar.gz with manifest to a file path", async () => {
    ...
    const { exportBackup } = await setupExport(stateDir);
  ...
  it("uses CORE_BACKUP_COMPONENTS by default", async () => {
    ...
    const result = await exportBackup({ output: outputPath });
  ...
  it("exports all 7 components when specified", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ version: 1, jobs: [{ id: "j1", schedule: "* * * * *" }] }),
    );
    const agentDir = path.join(stateDir, "agents", "default", "agent");
    await fs.writeFile(
    ...
    vi.doMock("../cron/store.js", () => ({
      DEFAULT_CRON_STORE_PATH: path.join(cronDir, "jobs.json"),
    }));
    const { exportBackup } = await import("./export.js");
...
describe("backup/export → import roundtrip", () => {
  ...
  it("encrypted roundtrip preserves data integrity", async () => {
    ...
    const result = await importBackup({ input: archivePath, decrypt: passphrase });
