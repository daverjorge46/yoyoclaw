...
export function resolveAssistantIdentity(params: {
  ...
  // Check per-agent config first (ui.agents.<agentId>)
  const agentsConfig = params.cfg.ui?.agents?.[agentId];
  const configAssistant = agentsConfig ?? params.cfg.ui?.assistant;
  const agentIdentity = resolveAgentIdentity(params.cfg, agentId);
  const fileIdentity = workspaceDir ? loadAgentIdentity(workspaceDir) : null;

  const name =
    coerceIdentityValue(agentsConfig?.name ?? configAssistant?.name, MAX_ASSISTANT_NAME) ??
  ...
  const avatarCandidates = [
    coerceIdentityValue(agentsConfig?.avatar ?? configAssistant?.avatar, MAX_ASSISTANT_AVATAR),
