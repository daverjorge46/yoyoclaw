...
function resolveCachePath(): string | null {
  const candidates = [
    ...
    path.resolve(process.env.HOME ?? "/root", ".openclaw/workspace/references/line-profile-cache.json"),
  ];
...
export async function resolveLineSenderNameAsync(
  ...
  if (!opts.getUserProfile) return undefined;

  // 3. 避免重複打 API
  if (_pendingLookups.has(userId)) return undefined;
