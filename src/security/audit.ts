...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("yoyoclaw security audit --deep")}".`,
    });
    return findings;
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("yoyoclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("yoyoclaw security audit --deep")}".`,
    });
  }
