...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  if (provider === "tavily") {
    return {
      ...
      message: `web_search (tavily) needs an API key. Run \`${formatCliCommand("openclaw configure --section web")}\` to store it, or set TAVILY_API_KEY in the Gateway environment.`,
      docs: "https://docs.openclaw.ai/tools/web",
    };
  }
  return {
...
async function runWebSearch(params: {
  ...
  if (params.provider === "tavily") {
    const res = await fetch(TAVILY_SEARCH_ENDPOINT, {
      ...
      body: JSON.stringify({
        query: params.query,
        max_results: Math.min(Math.max(params.count, 1), 20),
...
export function createWebSearchTool(options?: {
  ...
  const description =
    provider === "perplexity"
      ...
      : provider === "tavily"
        ? "Search the web using Tavily Search API. Returns titles, URLs, and snippets for fast research."
