...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("yoyoclaw models auth setup-token")}\` or \`${formatCliCommand("yoyoclaw configure")}\`.`
          : `Run \`${formatCliCommand("yoyoclaw configure")}\` or set an API key env var.`;
