...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "YoyoClaw update wizard" : "YoyoClaw configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("yoyoclaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
