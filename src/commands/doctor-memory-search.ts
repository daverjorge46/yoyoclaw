...
export async function noteMemorySearchHealth(cfg: OpenClawConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("yoyoclaw config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("yoyoclaw memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- To disable: ${formatCliCommand("yoyoclaw config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("yoyoclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("yoyoclaw auth add --provider openai")}`,
