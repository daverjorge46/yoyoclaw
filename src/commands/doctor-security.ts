...
export async function noteSecurityWarnings(cfg: OpenClawConfig) {
  ...
  const auditHint = `- Run: ${formatCliCommand("yoyoclaw security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("yoyoclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("yoyoclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("yoyoclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("yoyoclaw config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
