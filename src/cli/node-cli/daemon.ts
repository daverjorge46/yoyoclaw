...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("yoyoclaw node install"), formatCliCommand("yoyoclaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("yoyoclaw node install --force")}`);
    }
    return;
  }
