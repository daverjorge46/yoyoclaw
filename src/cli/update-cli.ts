...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update YoYo-Claw to the latest version")
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.yoyoclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.yoyoclaw.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
