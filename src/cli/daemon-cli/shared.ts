...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("yoyoclaw gateway install", env)}`);
    if (fileLog) {
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("yoyoclaw gateway install", env),
    formatCliCommand("yoyoclaw gateway", env),
  ];
