...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.yoyoclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + YoYo-Claw hooks")
    ...
    .option("--hook-url <url>", "YoYo-Claw hook URL")
    .option("--hook-token <token>", "YoYo-Claw hook token")
