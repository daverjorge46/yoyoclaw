...
describe("Nix integration (U3, U5, U9)", () => {
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.yoyoclaw when env not set", () => {
    ...
    it("STATE_DIR respects OPENCLAW_HOME when state override is unset", () => {
      ...
      ).toBe(path.join(path.resolve(customHome), ".yoyoclaw"));
    });

    it("CONFIG_PATH defaults to OPENCLAW_HOME/.yoyoclaw/yoyoclaw.json", () => {
      const customHome = path.join(path.sep, "custom", "home");
      expect(
        resolveConfigPathCandidate(
          envWith({
      ...
      ).toBe(path.join(path.resolve(customHome), ".yoyoclaw", "yoyoclaw.json"));
    });

    it("CONFIG_PATH defaults to ~/.yoyoclaw/yoyoclaw.json when env not set", () => {
      expect(
        resolveConfigPathCandidate(
          envWith({ OPENCLAW_CONFIG_PATH: undefined, OPENCLAW_STATE_DIR: undefined }),
        ),
      ).toMatch(/\.yoyoclaw[\\/]yoyoclaw\.json$/);
    });
    ...
    it("CONFIG_PATH expands ~ in OPENCLAW_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        expect(
          resolveConfigPathCandidate(
            envWith({ OPENCLAW_HOME: home, OPENCLAW_CONFIG_PATH: "~/.yoyoclaw/custom.json" }),
            () => home,
          ),
        ).toBe(path.join(home, ".yoyoclaw", "custom.json"));
      });
    });

    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", () => {
      expect(resolveConfigPathCandidate(envWith({ OPENCLAW_STATE_DIR: "/custom/state" }))).toBe(
        path.join(path.resolve("/custom/state"), "yoyoclaw.json"),
      );
    });
  });
  ...
  describe("U5b: tilde expansion for config paths", () => {
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
        const configDir = path.join(home, ".yoyoclaw");
        ...
        await fs.writeFile(
          path.join(configDir, "yoyoclaw.json"),
          JSON.stringify(
            {
              plugins: {
                load: {
              ...
              channels: {
                whatsapp: {
                  accounts: {
                    personal: {
                      authDir: "~/.yoyoclaw/credentials/wa-personal",
                    },
                  },
                },
              },
        ...
        expect(cfg.agents?.list?.[0]?.agentDir).toBe(
          path.join(home, ".yoyoclaw", "agents", "main"),
        );
        expect(cfg.agents?.list?.[0]?.sandbox?.workspaceRoot).toBe(path.join(home, "sandbox-root"));
        expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
          path.join(home, ".yoyoclaw", "credentials", "wa-personal"),
        );
      });
    });
  });
