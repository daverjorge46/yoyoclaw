...
This directory contains hooks that ship with YoyoClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
homepage: https://docs.yoyoclaw.ai/automation/hooks#my-hook
metadata:
...
Hooks can be configured in `~/.yoyoclaw/yoyoclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'yoyoclaw.*gateway' && pnpm yoyoclaw gateway`
3. Enable the hook: `yoyoclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.yoyoclaw.ai/automation/hooks
