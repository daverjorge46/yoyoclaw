...
homepage: https://docs.yoyoclaw.ai/automation/hooks#command-logger
metadata:
  {
    "openclaw":
      {
        "emoji": "📝",
        "events": ["command"],
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with YoyoClaw" }],
      },
  }
---
...
2. **Appends to log file** - Writes a JSON line to `~/.yoyoclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.yoyoclaw/logs/commands.log ~/.yoyoclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/yoyoclaw`:
   ```
