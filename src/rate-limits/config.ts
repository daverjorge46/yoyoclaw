...
export function resolveRateLimitsConfig(raw?: RawLimitsConfig): ResolvedLimitsConfig {
    ...
    const defaults: Required<RateLimitProviderConfig> = {
        ...
        dailyTokenBudget: positiveOrDefault(raw?.defaults?.dailyTokenBudget, DEFAULT_PROVIDER_LIMITS.dailyTokenBudget),
        monthlyTokenBudget: positiveOrDefault(raw?.defaults?.monthlyTokenBudget, DEFAULT_PROVIDER_LIMITS.monthlyTokenBudget),
    };
    ...
    const retry = {
        ...
        jitter: clamp01(raw?.retry?.jitter, DEFAULT_RETRY.jitter),
    };
