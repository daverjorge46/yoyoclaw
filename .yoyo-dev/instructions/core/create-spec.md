---
description: Spec Creation Rules for Yoyo Dev
globs:
alwaysApply: false
version: 1.1
encoding: UTF-8
...
<actions> 1. CHECK @.yoyo-dev/product/roadmap.md 2. FIND next uncompleted item 3. SUGGEST item to user 4. WAIT for approval
</actions>
</option_a_flow>
...
Use the context-fetcher subagent to read @.yoyo-dev/product/mission-lite.md and @.yoyo-dev/product/tech-stack.md only if not already in context to ensure minimal context for spec alignment.
...
### Step 5: Spec Folder and State File Creation

Use the file-creator subagent to create directory: .yoyo-dev/specs/YYYY-MM-DD-spec-name/ using the date from step 4, and initialize state.json for workflow tracking.
...
<file_path>.yoyo-dev/specs/YYYY-MM-DD-spec-name/state.json</file_path>
<initial_content>
{
"spec_name": "[SPEC_NAME]",
"spec_created": "[CURRENT_DATE]",
"tasks_created": null,
...
<purpose>Track workflow state across commands</purpose>
<note>Branch field removed - commits are made to current active branch</note>
</state_file_initialization>
...
Use the file-creator subagent to create the file: .yoyo-dev/specs/YYYY-MM-DD-spec-name/spec.md using this template:
...
Use the file-creator subagent to create the file: .yoyo-dev/specs/YYYY-MM-DD-spec-name/spec-lite.md for the purpose of establishing a condensed spec for efficient AI context usage.
...
<file_template>
  ...
  <header>
    ...
    This is the technical specification for the spec detailed in @.yoyo-dev/specs/YYYY-MM-DD-spec-name/spec.md
...
<file_template>
  ...
  <header>
    ...
    This is the database schema implementation for the spec detailed in @.yoyo-dev/specs/YYYY-MM-DD-spec-name/spec.md
...
<file_template>
  ...
  <header>
    ...
    This is the API specification for the spec detailed in @.yoyo-dev/specs/YYYY-MM-DD-spec-name/spec.md
...
<step number="11" subagent="file-creator" name="create_decisions_log">

### Step 11: Create Decisions Log

Use the file-creator subagent to create the file: .yoyo-dev/specs/YYYY-MM-DD-spec-name/decisions.md for tracking technical decisions and rationale.
...
<initial_decisions>
Include key decisions from spec creation:

- Tech stack choices from technical-spec.md
- Database schema decisions (if applicable)
- API design decisions (if applicable)
- Architecture patterns selected
  </initial_decisions>

<instructions>
  ACTION: Create decisions.md with header
  POPULATE: Add 1-3 initial decisions from spec creation
  FORMAT: Use decision_entry_template for each
  PURPOSE: Preserve reasoning for execution phase
</instructions>
...
- Spec Requirements: @.yoyo-dev/specs/YYYY-MM-DD-spec-name/spec.md
- Spec Summary: @.yoyo-dev/specs/YYYY-MM-DD-spec-name/spec-lite.md
- Technical Spec: @.yoyo-dev/specs/YYYY-MM-DD-spec-name/sub-specs/technical-spec.md
- Technical Decisions: @.yoyo-dev/specs/YYYY-MM-DD-spec-name/decisions.md
