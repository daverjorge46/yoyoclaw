...
export async function handleWhatsAppStart(host: YoYoClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: YoYoClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: YoYoClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: YoYoClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: YoYoClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: YoYoClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: YoYoClawApp): string | null {
...
function buildGatewayHttpHeaders(host: YoYoClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: YoYoClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: YoYoClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: YoYoClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: YoYoClawApp) {
  const state = host.nostrProfileFormState;
