    ...
    <section class="card">
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          pending.length > 0
            ? html`
              <div class="muted" style="margin-bottom: 8px;">${t("common.pending")}</div>
        ...
        ${
          paired.length > 0
            ? html`
              <div class="muted" style="margin-top: 12px; margin-bottom: 8px;">${t("common.paired")}</div>
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const scopes = `${t("instances.scopes")}: ${formatList(device.scopes)}`;
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs ? t("nodes.revoked") : t("common.active");
  const scopes = `${t("instances.scopes")}: ${formatList(token.scopes)}`;
...
const SECURITY_OPTIONS: Array<{ value: ExecSecurity; label: string }> = [
  ...
  { value: "allowlist", label: t("nodes.allowlistLabel") },
  { value: "full", label: t("nodes.full") },
];

const ASK_OPTIONS: Array<{ value: ExecAsk; label: string }> = [
  { value: "off", label: t("nodes.off") },
  { value: "on-miss", label: t("nodes.onMiss") },
  { value: "always", label: t("nodes.always") },
];
  ...
  return html`
    <section class="card">
      ...
      ${
        !ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("nodes.loadApprovals")}</div>
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? t("common.loading") : t("nodes.loadApprovalsBtn")}
            </button>
          </div>`
          : html`
...
function renderExecApprovalsTarget(state: ExecApprovalsState) {
  ...
  return html`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("common.target")}</div>
          <div class="list-sub">
            ${t("nodes.targetSub")}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              ...
              <option value="node" ?selected=${state.target === "node"}>${t("common.node")}</option>
            </select>
          </label>
          ${
            state.target === "node"
      ...
      ${
        state.target === "node" && !hasNodes
          ? html`
              <div class="muted">${t("nodes.noExecApprovalsNodes")}</div>
            `
          : nothing
      }
    </div>
    ...
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${isDefaults ? t("nodes.defaultSecurityMode") : t("nodes.securityDefault").replace("{value}", defaults.security)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${securityValue === "__default__"}>
                    ${t("nodes.useDefault").replace("{value}", defaults.security)}
                  </option>`
                  : nothing
              }
      ...
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${isDefaults ? t("nodes.defaultPromptPolicy") : t("nodes.askDefault").replace("{value}", defaults.ask)}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${askValue === "__default__"}>
                    ${t("nodes.useDefault").replace("{value}", defaults.ask)}
                  </option>`
                  : nothing
              }
              ${ASK_OPTIONS.map(
          ...
          <div class="list-sub">
            ${
              isDefaults
                ...
                : t("nodes.securityDefault").replace("{value}", defaults.askFallback)
            }
          </div>
        </div>
        <div class="list-meta">
              ...
              ${
                !isDefaults
                  ? html`<option value="__default__" ?selected=${askFallbackValue === "__default__"}>
                    ${t("nodes.useDefault").replace("{value}", defaults.askFallback)}
                  </option>`
                  : nothing
              }
              ${SECURITY_OPTIONS.map(
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        ...
        <div class="card-sub">${t("nodes.allowlistSub")}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? t("common.paired") : "unpaired"}</span>
