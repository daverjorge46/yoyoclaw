...
import { YoYoClawApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = YoYoClawApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("yoyoclaw-app") as YoYoClawApp;
...
beforeEach(() => {
  YoYoClawApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__YOYOCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  YoYoClawApp.prototype.connect = originalConnect;
  window.__YOYOCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("honors explicit base path overrides", async () => {
    window.__YOYOCLAW_CONTROL_UI_BASE_PATH__ = "/openclaw";
