...
export function formatNextRun(ms?: number | null) {
  if (!ms) return t("common.n_a");
  return `${formatMs(ms)} (${formatAgo(ms)})`;
}

export function formatSessionTokens(row: GatewaySessionRow) {
  if (row.totalTokens == null) return t("common.n_a");
...
export function formatCronSchedule(job: CronJob) {
  ...
  if (s.kind === "at") return `${t("cron.schedule_at")} ${formatMs(s.atMs)}`;
  if (s.kind === "every") return `${t("cron.schedule_every")} ${formatDurationMs(s.everyMs)}`;
  return `${t("cron.schedule_cron")} ${s.expr}${s.tz ? ` (${s.tz})` : ""}`;
}

export function formatCronPayload(job: CronJob) {
  const p = job.payload;
  if (p.kind === "systemEvent") return `${t("cron.payload_system")}: ${p.text}`;
  return `${t("cron.payload_agent")}: ${p.message}`;
}
