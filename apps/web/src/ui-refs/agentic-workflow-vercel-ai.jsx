...
const Dropdown = ({ value, onChange, options, label, renderOption, className = '' }) => {
...
const Toggle = ({ enabled, onChange, label, size = 'sm' }) => {
  const sizes = {
...
const ToolApprovalCard = ({ toolCall, onApprove, onReject, onModify, status = 'pending' }) => {
...
const QuestionCard = ({ question, onSubmit, status = 'pending' }) => {
...
const UserMessage = ({ content, attachments, timestamp }) => (
...
const AssistantMessage = ({ content, isStreaming, reasoning, timestamp }) => {
...
const ThinkingIndicator = ({ thought }) => (
  <div className="flex gap-2">
    <Avatar type="assistant" size="sm" />
    <div className="bg-gray-800/50 border border-gray-700/50 px-3 py-2 rounded-xl rounded-bl-sm">
      ...
      <div className="flex gap-1 mt-1.5">
        {[0, 1, 2].map(i => (
          <div key={i} className="w-1.5 h-1.5 bg-amber-500 rounded-full animate-bounce" style={{ animationDelay: `${i * 150}ms` }} />
        ))}
      </div>
    </div>
  </div>
...
const SystemMessage = ({ type, content }) => {
  const config = {
...
const CommandPalette = ({ isOpen, onClose, onCommand }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]">
      ...
      <div className="relative w-full max-w-md bg-gray-900 rounded-lg border border-gray-700 shadow-2xl overflow-hidden" onKeyDown={handleKeyDown}>
        ...
        <div className="max-h-[280px] overflow-y-auto">
          {filtered.map((cmd, i) => (
            ...
            >
              <cmd.icon className={`w-3.5 h-3.5 ${i === selectedIndex ? 'text-blue-400' : 'text-gray-500'}`} />
...
const ToolPermissionPanel = ({ isOpen, onClose }) => {
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-gray-900 rounded-xl border border-gray-700 shadow-2xl overflow-hidden max-h-[80vh] flex flex-col">
        ...
        <div className="flex border-b border-gray-800">
          {['tiers', 'tools', 'exceptions'].map(tab => (
            <button
              key={tab}
              ...
              {AVAILABLE_TOOLS.map(tool => {
                ...
                return (
                  <div key={tool.id} className="flex items-center gap-2 px-2 py-1.5 bg-gray-800 rounded">
                    <cat.icon className={`w-3 h-3 text-${cat.color}-400`} />
                    <span className="text-[11px] text-gray-200 flex-1">{tool.name}</span>
                    <risk.icon className={`w-3 h-3 text-${risk.color}-400`} />
                  </div>
                );
              })}
            </div>
...
const InputArea = ({ onSend, disabled, onStop }) => {
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          {attachments.map(att => (
            <div key={att.id} className="relative group">
              {att.type?.startsWith('image') ? (
                <img src={att.preview} alt="" className="h-12 w-12 object-cover rounded border border-gray-700" />
              ) : (
...
export default function AgenticWorkflow() {
  ...
  const [toolPermissions, setToolPermissions] = useState({ low: 'auto', medium: 'ask', high: 'ask' });
  ...
  // Handle tool calls
  const handleToolCalls = (toolInvocations) => {
    toolInvocations.forEach(tc => {
      ...
      if (autoApprove || permission === 'auto') {
        ...
        // Add to pending
        setPendingToolCalls(prev => [...prev, { ...tc, status: 'pending' }]);
  ...
  const executeToolCall = async (toolCall) => {
    ...
    // In real implementation, this would use the Vercel AI SDK's tool result handling
    setWorkflowStatus('thinking');
  };
  ...
  const askQuestion = (question) => {
    const q = { id: generateId(), ...question, status: 'pending' };
