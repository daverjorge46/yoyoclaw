...
import { MessageCircle, Settings, Play, Pause } from "lucide-react";
      ...
      >
        <Card className="overflow-hidden border-border/50 bg-card/80 backdrop-blur-sm transition-all duration-300 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5">
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <Button variant="ghost" size="icon" onClick={onChat} className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <Card className="relative overflow-hidden rounded-2xl border-border/50 bg-gradient-to-br from-card via-card to-card/80 backdrop-blur-sm transition-all duration-500 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/10">
        ...
        <CardContent className="relative p-6">
          ...
          <div className="flex gap-2">
            <Button onClick={onChat} className="flex-1 h-11 rounded-xl bg-primary/10 text-primary hover:bg-primary/20 transition-all" variant="ghost">
              <MessageCircle className="mr-2 h-4 w-4" />
              Chat
            </Button>
            <Button onClick={onToggle} variant="ghost" size="icon" className="h-11 w-11 rounded-xl bg-secondary/50 hover:bg-secondary transition-all">
              {agent.status === "paused" ? <Play className="h-4 w-4" /> : <Pause className="h-4 w-4" />}
            </Button>
            <Button onClick={onSettings} variant="ghost" size="icon" className="h-11 w-11 rounded-xl bg-secondary/50 hover:bg-secondary transition-all">
              <Settings className="h-4 w-4" />
            </Button>
          </div>
          {agent.lastActive && (
            <p className="mt-4 text-center text-xs text-muted-foreground/70">Last active {agent.lastActive}</p>
          )}
        </CardContent>
      </Card>
    </motion.div>
