...
This document describes how YoyoClaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

YoyoClaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, YoyoClaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in YoyoClaw's local TUI mode)                                             |
...
├── pi-tools.ts                    # createYoyoClawCodingTools()
...
├── yoyoclaw-tools.ts              # YoyoClaw-specific tools
...
2. **Custom Replacements**: YoyoClaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **YoyoClaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures YoyoClaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, YoyoClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
YoyoClaw wraps this with `guardSessionManager()` for tool result safety.
...
YoyoClaw maintains an auth profile store with multiple API keys per provider:
...
YoyoClaw loads custom pi extensions for specialized behavior:
...
YoyoClaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | YoyoClaw Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom YoyoClaw tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.yoyoclaw/agents/<agentId>/sessions/` (or `$OPENCLAW_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-yoyoclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-yoyoclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-yoyoclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-yoyoclaw-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
