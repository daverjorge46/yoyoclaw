...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.yoyoclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.yoyoclaw/node.json` (node id + pairing token).
...
Path: `~/.yoyoclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.yoyoclaw/exec-approvals.sock",
...
- Unix socket at `~/.yoyoclaw/exec-approvals.sock` (0600).
