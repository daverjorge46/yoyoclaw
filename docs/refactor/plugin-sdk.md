...
- Publish as `yoyoclaw/plugin-sdk` (or export from core under `yoyoclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `YoyoClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: YoyoClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: YoyoClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: YoyoClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: YoyoClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: YoyoClawConfig): string;
  };
};
```
...
- Introduce `yoyoclaw/plugin-sdk`.
- Add `api.runtime` to `YoyoClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `yoyoclawRuntime: ">=2026.2.0"`).
