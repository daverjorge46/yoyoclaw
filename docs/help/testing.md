...
YoyoClaw has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  - YoyoClaw uses Vitest `vmForks` on Node 22/23 for faster unit shards.
  - On Node 24+, YoyoClaw automatically falls back to regular `forks` to avoid Node VM linking errors (`ERR_VM_MODULE_LINK_FAILURE` / `module is already linked`).
...
### Layer 2: Gateway + dev agent smoke (what “@yoyoclaw” actually does)
...
```bash
yoyoclaw models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
...
- Gemini API vs Gemini CLI:
  - API: YoyoClaw calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: YoyoClaw shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `yoyoclaw models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `yoyoclaw models list` / model selection.

- Profile store: `~/.yoyoclaw/credentials/` (preferred; what “profile keys” means in the tests)
...
- `OPENCLAW_CONFIG_DIR=...` (default: `~/.yoyoclaw`) mounted to `/home/node/.yoyoclaw`
- `OPENCLAW_WORKSPACE_DIR=...` (default: `~/.yoyoclaw/workspace`) mounted to `/home/node/.yoyoclaw/workspace`
