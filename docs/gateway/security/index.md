...
## Quick check: `yoyoclaw security audit`
...
```bash
yoyoclaw security audit
yoyoclaw security audit --deep
yoyoclaw security audit --fix
```
...
- Tighten local perms (`~/.yoyoclaw` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

YoyoClaw is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, YoyoClaw also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.yoyoclaw/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.yoyoclaw/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.yoyoclaw/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.yoyoclaw/credentials/oauth.json`
...
`yoyoclaw security audit` warns when this setting is enabled.
...
YoyoClaw stores session transcripts on disk under `~/.yoyoclaw/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.yoyoclaw` (see the audit section below). If you need
...
YoyoClaw can refresh the skills list mid-session:
...
- If you install plugins from npm (`yoyoclaw plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.yoyoclaw/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
  - YoyoClaw uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
yoyoclaw pairing approve <channel> <code>
```
...
By default, YoyoClaw routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
YoyoClaw has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.yoyoclaw/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.yoyoclaw or your logs.”
...
4. **Re-run audit**
   - `yoyoclaw security audit --deep` and confirm the report is clean.
...
- `~/.daverjorge46/yoyoclaw.json`: `600` (user read/write only)
- `~/.yoyoclaw`: `700` (user only)

`yoyoclaw doctor` can warn and offer to tighten these permissions.
...
- Canvas host: `/__yoyoclaw__/canvas/` and `/__yoyoclaw__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_yoyoclaw-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
Doctor can generate one for you: `yoyoclaw doctor --generate-gateway-token`.
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), YoyoClaw
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. YoyoClaw verifies the identity by resolving the
...
- YoyoClaw will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.yoyoclaw/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:

- `yoyoclaw.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `yoyoclaw status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@yoyoclaw", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.yoyoclaw/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `yoyoclaw` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only YoyoClaw clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `yoyoclaw gateway` process.
...
1. Check Gateway logs: `/tmp/daverjorge46/yoyoclaw-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.yoyoclaw/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + YoyoClaw version
...
Found a vulnerability in YoyoClaw? Please report responsibly:
