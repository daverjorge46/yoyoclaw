...
read_when:
  - Running YoyoClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of YoyoClaw
...
- You run YoyoClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. YoyoClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. YoyoClaw extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://yoyoclaw.example.com
    to: http://yoyoclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
yoyoclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy yoyoclaw:18789 {
...
location / {
    ...
    proxy_pass http://yoyoclaw:18789;
...
`yoyoclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update YoyoClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `yoyoclaw security audit` and review findings
