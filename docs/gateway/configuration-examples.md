...
## Config includes (multi-file setup)

Use `$include` to split a large config across files. This example shows a multi-client legal setup:
...
See [Configuration](/gateway/configuration#split-config-into-multiple-files-include) for the full `$include` rules.
