...
- During heartbeat runs, YoyoClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `yoyoclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, YoyoClaw skips the heartbeat run entirely (no model call).
...
```bash
yoyoclaw system event --text "Check for urgent follow-ups" --mode now
```
