...
description: "Complete field-by-field reference for ~/.daverjorge46/yoyoclaw.json"
---

# Configuration Reference

Every field available in `~/.daverjorge46/yoyoclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — YoyoClaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `yoyoclaw doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["yoyoclaw-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `yoyoclaw plugins install @yoyoclaw/mattermost`.
...
YoyoClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@yoyoclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `yoyoclaw.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, YoyoClaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, YoyoClaw assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.yoyoclaw/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.yoyoclaw/workspace",
        agentDir: "~/.yoyoclaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@yoyoclaw"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:yoyoclaw`   | All built-in tools (excludes provider plugins)                                           |
...
YoyoClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.yoyoclaw/agents/<agentId>/agent/models.json`.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `yoyoclaw onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `yoyoclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `yoyoclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `yoyoclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `yoyoclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `yoyoclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.yoyoclaw/extensions`, `<workspace>/.yoyoclaw/extensions`, plus `plugins.load.paths`.
...
Convenience flags: `--dev` (uses `~/.yoyoclaw-dev` + port `19001`), `--profile <name>` (uses `~/.yoyoclaw-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.yoyoclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-yoyoclaw-token: <token>`.
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__yoyoclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__yoyoclaw__/a2ui/`
...
- Also serves A2UI at `/__yoyoclaw__/a2ui/`.
...
- Hostname defaults to `yoyoclaw`. Override with `OPENCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.yoyoclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `yoyoclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.yoyoclaw/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.yoyoclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/daverjorge46/yoyoclaw-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `yoyoclaw doctor --fix` can strip unknown keys).
