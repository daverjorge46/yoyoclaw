...
Deploy YoyoClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/daverjorge46/yoyoclaw)
...
repository configures everything needed to run YoyoClaw:
...
The web dashboard is available at `https://<your-service>.onrender.com/yoyoclaw`.
...
If you use the original YoyoClaw repository, Render will not auto-deploy your YoyoClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For YoyoClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any YoyoClaw host.
