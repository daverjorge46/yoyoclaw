...
Deploy YoyoClaw on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy YoyoClaw](https://northflank.com/stacks/deploy-yoyoclaw) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy YoyoClaw now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the YoyoClaw template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the YoyoClaw service.
8. Open the public YoyoClaw URL and complete setup at `/setup`.
9. Open the Control UI at `/yoyoclaw`.

## What you get

- Hosted YoyoClaw Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/yoyoclaw`
