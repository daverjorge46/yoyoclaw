---
title: Fly.io
description: Deploy YoyoClaw on Fly.io
---

# Fly.io Deployment

**Goal:** YoyoClaw Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-yoyoclaw

# Create a persistent volume (1GB is usually enough)
fly volumes create yoyoclaw_data --size 1 --region iad
```
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `yoyoclaw.json` where they could be accidentally exposed or logged.
...
**Note:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/yoyoclaw.json`.
...
Or visit `https://my-yoyoclaw.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/yoyoclaw.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/yoyoclaw.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/yoyoclaw.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/yoyoclaw.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/yoyoclaw.json"
```
...
```bash
# List current IPs
fly ips list -a my-yoyoclaw

# Release public IPs
fly ips release <public-ipv4> -a my-yoyoclaw
fly ips release <public-ipv6> -a my-yoyoclaw
...
fly ips allocate-v6 --private -a my-yoyoclaw
```
...
fly proxy 3000:3000 -a my-yoyoclaw
