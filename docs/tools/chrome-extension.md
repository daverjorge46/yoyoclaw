---
summary: "Chrome extension: let YoyoClaw drive your existing Chrome tab"
read_when:
...
The YoyoClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate yoyoclaw-managed Chrome profile.
...
YoyoClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the YoyoClaw release (npm package) as static files. There is no separate “build” step.

After upgrading YoyoClaw:

- Re-run `yoyoclaw browser extension install` to refresh the installed files under your YoyoClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

YoyoClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want YoyoClaw to control.
...
- `ON`: attached; YoyoClaw can drive that tab.
...
Debugging: `yoyoclaw sandbox explain`
...
`yoyoclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `yoyoclaw browser extension install` first to copy the extension to a stable location under your YoyoClaw state directory.
...
- **This is not isolated** like the dedicated yoyoclaw-managed profile.
