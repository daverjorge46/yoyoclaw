...
- **gateway host** → `yoyoclaw` process on the gateway machine
...
`~/.yoyoclaw/exec-approvals.json` directly.

CLI: `yoyoclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
