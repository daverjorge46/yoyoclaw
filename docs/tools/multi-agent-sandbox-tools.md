...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `yoyoclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `yoyoclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=yoyoclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENCLAW_STATE_DIR:-$HOME/.yoyoclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
