---
summary: "Direct `yoyoclaw agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `yoyoclaw agent` (direct agent runs)

`yoyoclaw agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `yoyoclaw message --target`).
...
```bash
yoyoclaw agent --to +15555550123 --message "status update"
...
yoyoclaw agent --to +15555550123 --message "Summon reply" --deliver
