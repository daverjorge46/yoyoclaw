---
summary: "Agent tool surface for YoyoClaw (browser, canvas, nodes, message, cron) replacing legacy `yoyoclaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `yoyoclaw-*` skills
title: "Tools"
---

# Tools (YoyoClaw)

YoyoClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `yoyoclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `yoyoclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, YoyoClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:yoyoclaw`: all built-in YoyoClaw tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`yoyoclaw node run`).
...
- Requires a Brave API key (recommended: `yoyoclaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated YoyoClaw-managed browser.
...
- Quick smoke: `yoyoclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `yoyoclaw gateway` restart in-place)
