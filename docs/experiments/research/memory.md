...
read_when:
  - Designing workspace memory (~/.yoyoclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep YoyoClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.yoyoclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.yoyoclaw/workspace` as canonical human-readable memory.
...
```
~/.yoyoclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in YoyoClaw**, but keep a separable core library.

### Why integrate into YoyoClaw?

- YoyoClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `yoyoclaw memory recall "…" --k 25 --since 30d`
  - `yoyoclaw memory reflect --since 7d`
...
Pragmatic take for `~/.yoyoclaw/workspace`:
