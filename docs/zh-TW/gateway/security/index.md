...
1. **公開的網路暴露**（LAN 綁定、Funnel、缺少身分驗證）：立即修正。
1. **瀏覽器控制的遠端暴露**：視同操作員存取（僅限 tailnet、刻意配對節點、避免公開暴露）。
1. **權限**：確保狀態／設定／憑證／身分驗證不是群組或世界可讀。
1. **外掛／擴充**：只載入你明確信任的項目。
1. **模型選擇**：任何具備工具的機器人，優先使用現代、指令強化的模型。
...
23. `/exec` 是提供給已授權操作人員的僅限工作階段便利功能。 26. 它 **不會** 寫入設定或變更其他工作階段。
