...
`/prose run <handle/slug>` จะถูกแก้ไขเป็น `https://p.prose.md/<handle>/<slug>`. 28. URL โดยตรงจะถูกดึงมาใช้งานตามสภาพเดิม URL แบบตรงจะถูกดึงตามที่เป็นอยู่ ใช้เครื่องมือ `web_fetch` (หรือ `exec` สำหรับ POST)
