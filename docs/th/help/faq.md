...
16. หากต้องการเผื่อทรัพยากรเพิ่ม (ล็อก สื่อ บริการอื่น) **แนะนำ 2GB**, แต่ไม่ใช่ขั้นต่ำที่บังคับ

17. เคล็ดลับ: Pi/VPS ขนาดเล็กสามารถโฮสต์ Gateway ได้ และคุณสามารถจับคู่ **nodes** บนแล็ปท็อป/โทรศัพท์เพื่อใช้หน้าจอ/กล้อง/แคนวาสภายในเครื่องหรือสั่งรันคำสั่ง 19. ดู [Nodes](/nodes).
...
1. 41. คัดลอก `$OPENCLAW_STATE_DIR` (ค่าเริ่มต้น: `~/.openclaw`) จากเครื่องเดิม
1. 42. คัดลอก workspace ของคุณ (ค่าเริ่มต้น: `~/.openclaw/workspace`)
1. 43. รัน `openclaw doctor` แล้วรีสตาร์ตบริการ Gateway
...
47. ที่เกี่ยวข้อง: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-openclaw-store-its-data),
...
2. ดู [OAuth](/concepts/oauth), [Model providers](/concepts/model-providers) และ [Wizard](/start/wizard)
...
8. 8. เปิดใช้งานปลั๊กอิน: `openclaw plugins enable google-gemini-cli-auth`
9. เข้าสู่ระบบ: `openclaw models auth login --provider google-gemini-cli --set-default`
...
22. คุณต้องใช้ Mac **เฉพาะสำหรับเครื่องมือที่เป็น macOS-only** เท่านั้น 25. สำหรับ iMessage ให้ใช้ [BlueBubbles](/channels/bluebubbles) (แนะนำ) — เซิร์ฟเวอร์ BlueBubbles รันบน Mac ใดก็ได้ และ Gateway สามารถรันบน Linux หรือที่อื่นได้ 26. หากต้องการใช้เครื่องมือ macOS-only อื่น ๆ ให้รัน Gateway บน Mac หรือจับคู่กับโหนด macOS

23. เอกสาร: [BlueBubbles](/channels/bluebubbles), [Nodes](/nodes), [Mac remote mode](/platforms/mac/remote)
...
47. หากยังอยากทดลองใช้ Bun ให้ทำบน gateway ที่ไม่ใช้งานจริง (non-production)
...
10. สำหรับ **outreach หรือการรันโฆษณา** ควรมีมนุษย์อยู่ในวงการทำงาน 12. หลีกเลี่ยงสแปม ปฏิบัติตามกฎหมายท้องถิ่นและ
    นโยบายของแพลตฟอร์ม และตรวจทานทุกอย่างก่อนส่ง 13. รูปแบบที่ปลอดภัยที่สุดคือให้
    OpenClaw ร่าง แล้วคุณเป็นผู้อนุมัติ

11. เอกสาร: [Security](/gateway/security).
...
44. ขอให้บอตของคุณ "spawn a sub-agent for this task" หรือใช้ `/subagents`
45. ใช้ `/status` ในแชตเพื่อดูว่า Gateway กำลังทำอะไรอยู่ตอนนี้ (และกำลังยุ่งอยู่หรือไม่)

46. เคล็ดลับเรื่องโทเคน: งานที่ยาวและ sub-agents ต่างก็ใช้โทเคน 48. หากค่าใช้จ่ายเป็นข้อกังวล ให้ตั้งค่า
    โมเดลที่ถูกกว่าสำหรับ sub-agents ผ่าน `agents.defaults.subagents.model`

47. เอกสาร: [Sub-agents](/tools/subagents).
...
7. เอกสาร: [Memory](/concepts/memory), [Context](/concepts/context)
...
21. การตั้งค่าที่แนะนำ:

22. 23. **ตรวจสอบให้แน่ใจว่า VPS + Mac อยู่ใน tailnet เดียวกัน**
23. 24. **ใช้แอป macOS ในโหมด Remote** (เป้าหมาย SSH สามารถเป็นชื่อโฮสต์ของ tailnet ได้)
    25. แอปจะทำการ tunnel พอร์ต Gateway และเชื่อมต่อเป็นโหนด
24. 26. **อนุมัติโหนด** บน gateway:
...
1. 37. เปิดใช้งานการนำเข้า shell (ความสะดวกแบบ opt-in):
...
41. หาก Gateway รันเป็นบริการ (launchd/systemd) จะไม่ได้รับสภาพแวดล้อมจาก shell ของคุณ 43. แก้ไขโดยทำอย่างใดอย่างหนึ่งต่อไปนี้:

42. 44. ใส่โทเค็นใน `~/.openclaw/.env`:

    ```
    45. COPILOT_GITHUB_TOKEN=...
    ```

43. 46. หรือเปิดใช้งานการนำเข้า shell (`env.shellEnv.enabled: true`).

44. 47. หรือเพิ่มลงในบล็อก `env` ของคอนฟิก (จะมีผลเฉพาะเมื่อยังขาดอยู่)
...
1. **การสลับโมเดลสำรอง** ไปยังโมเดลถัดไปใน `agents.defaults.model.fallbacks`
...
47. วิธีแก้: ให้ตั้งค่า Google auth หรือเอา/หลีกเลี่ยงโมเดล Google ใน `agents.defaults.model.fallbacks` / aliases เพื่อไม่ให้ fallback วิ่งไปทางนั้น

48. **ข้อความ LLM request rejected message thinking signature required google antigravity**

49. สาเหตุ: ประวัติของเซสชันมี **thinking blocks ที่ไม่มี signature** (มักเกิดจากสตรีมที่ถูกยกเลิกหรือไม่สมบูรณ์) Google Antigravity ต้องการลายเซ็นสำหรับบล็อกการคิด (thinking blocks)
...
1. 46. อย่าเปิดพอร์ต WS ในแท็บเบราว์เซอร์ปกติ
1. 47. หากเปิดใช้ auth ให้ใส่โทเค็น/รหัสผ่านในเฟรม `connect`
