---
summary: "YoyoClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up YoyoClaw on a Raspberry Pi
  - Running YoyoClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# YoyoClaw on Raspberry Pi

## Goal

Run a persistent, always-on YoyoClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most YoyoClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled yoyoclaw

# Enable if not
sudo systemctl enable yoyoclaw

# Start on boot
sudo systemctl start yoyoclaw
```
...
```bash
# Check logs
journalctl -u yoyoclaw --no-pager -n 100

# Common fix: rebuild
cd ~/yoyoclaw  # if using hackable install
npm run build
sudo systemctl restart yoyoclaw
```
