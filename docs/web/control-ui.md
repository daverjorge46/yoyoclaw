...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/yoyoclaw`)
...
If the page fails to load, start the Gateway first: `yoyoclaw gateway`.
...
you revoke it with `yoyoclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.daverjorge46/yoyoclaw.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. YoyoClaw
...
YoyoClaw **blocks** Control UI connections without device identity.
