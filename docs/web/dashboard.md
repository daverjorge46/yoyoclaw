...
- Re-open anytime: `yoyoclaw dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
...
- Ensure the gateway is reachable (local: `yoyoclaw status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
- Retrieve the token from the gateway host: `yoyoclaw config get gateway.auth.token` (or generate one: `yoyoclaw doctor --generate-gateway-token`).
