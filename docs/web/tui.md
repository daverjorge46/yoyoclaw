...
```bash
yoyoclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `yoyoclaw tui --deliver`
...
- Check the Gateway logs: `yoyoclaw logs --follow`.
- Confirm the agent can run: `yoyoclaw status` and `yoyoclaw models status`.
...
- No agents in picker: check `yoyoclaw agents list` and your routing config.
