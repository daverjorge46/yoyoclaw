---
summary: "Use Venice AI privacy-focused models in YoyoClaw"
read_when:
  - You want privacy-focused inference in YoyoClaw
...
```bash
yoyoclaw onboard --auth-choice venice-api-key
```
...
```bash
yoyoclaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, YoyoClaw shows all available Venice models. Pick based on your needs:
...
```bash
yoyoclaw models set venice/claude-opus-45
yoyoclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
yoyoclaw models list | grep venice
```

## Configure via `yoyoclaw configure`

1. Run `yoyoclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
YoyoClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
yoyoclaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
yoyoclaw chat --model venice/claude-opus-45

# Use uncensored model
yoyoclaw chat --model venice/venice-uncensored

# Use vision model with image
yoyoclaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
yoyoclaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `yoyoclaw models list` to see currently available models. Some models may be temporarily offline.
