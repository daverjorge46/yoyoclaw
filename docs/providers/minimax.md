---
summary: "Use MiniMax M2.1 in YoyoClaw"
read_when:
  - You want MiniMax models in YoyoClaw
...
```bash
yoyoclaw plugins enable minimax-portal-auth  # skip if already loaded.
yoyoclaw gateway restart  # restart if gateway is already running
yoyoclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/daverjorge46/yoyoclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `yoyoclaw.json`:
...
- Use `yoyoclaw models list` and `yoyoclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `yoyoclaw configure` and selecting **MiniMax M2.1**, or
