...
read_when:
  ...
  - 你在编写提供商认证或部署环境的文档
summary: YoyoClaw 从哪里加载环境变量以及优先级顺序
title: 环境变量
x-i18n:
...
YoyoClaw 从多个来源拉取环境变量。规则是**永不覆盖现有值**。

## 优先级（从高到低）

1. **进程环境**（Gateway 网关进程从父 shell/守护进程已有的内容）。
2. **当前工作目录中的 `.env`**（dotenv 默认；不覆盖）。
3. **全局 `.env`** 位于 `~/.yoyoclaw/.env`（即 `$OPENCLAW_STATE_DIR/.env`；不覆盖）。
4. **配置 `env` 块** 位于 `~/.daverjorge46/yoyoclaw.json`（仅在缺失时应用）。
