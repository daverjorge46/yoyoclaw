...
当你需要切换不常用的设置而不编辑 `yoyoclaw.json` 时，这非常方便。
...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.yoyoclaw-dev` 下，并将 Gateway 网关端口默认为 `19001`（派生端口随之移动）。
...
如果你还没有全局安装，请通过 `pnpm yoyoclaw ...` 运行 CLI。
...
YoyoClaw 可以在任何过滤/格式化之前记录**原始助手流**。
