...
YoyoClaw 包含三个 Vitest 测试套件（单元/集成、端到端、实时）以及一小组 Docker 运行器。
...
### 第二层：Gateway 网关 + 开发智能体冒烟测试（"@yoyoclaw"实际做的事）
...
- Gemini API 与 Gemini CLI：
  - API：YoyoClaw 通过 HTTP 调用 Google 托管的 Gemini API（API 密钥/配置文件认证）；这是大多数用户说的"Gemini"。
  - CLI：YoyoClaw 调用本地 `gemini` 二进制文件；它有自己的认证，行为可能不同（流式传输/工具支持/版本差异）。
...
- OpenRouter：`openrouter/...`（数百个模型；使用 `yoyoclaw models scan` 查找支持工具+图像的候选模型）
...
- 如果实时测试说"无凭证"，用调试 `yoyoclaw models list`/模型选择相同的方式调试。

- 配置文件存储：`~/.yoyoclaw/credentials/`（首选；测试中"配置文件密钥"的含义）
...
- `OPENCLAW_CONFIG_DIR=...`（默认：`~/.yoyoclaw`）挂载到 `/home/node/.yoyoclaw`
- `OPENCLAW_WORKSPACE_DIR=...`（默认：`~/.yoyoclaw/workspace`）挂载到 `/home/node/.yoyoclaw/workspace`
