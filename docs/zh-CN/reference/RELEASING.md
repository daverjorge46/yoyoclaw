...
- [ ] 更新 CLI/版本字符串：[`src/cli/program.ts`](https://github.com/daverjorge46/yoyoclaw/blob/main/src/cli/program.ts) 和 [`src/provider-web.ts`](https://github.com/daverjorge46/yoyoclaw/blob/main/src/provider-web.ts) 中的 Baileys user agent。
- [ ] 确认包元数据（name、description、repository、keywords、license）以及 `bin` 映射指向 [`yoyoclaw.mjs`](https://github.com/daverjorge46/yoyoclaw/blob/main/yoyoclaw.mjs) 作为 `yoyoclaw`。
- [ ] 如果依赖项有变化，运行 `pnpm install` 确保 `pnpm-lock.yaml` 是最新的。

2. **构建和产物**

- [ ] 如果 A2UI 输入有变化，运行 `pnpm canvas:a2ui:bundle` 并提交更新后的 [`src/canvas-host/a2ui/a2ui.bundle.js`](https://github.com/daverjorge46/yoyoclaw/blob/main/src/canvas-host/a2ui/a2ui.bundle.js)。
...
- [ ] 生成 Sparkle appcast（通过 [`scripts/make_appcast.sh`](https://github.com/daverjorge46/yoyoclaw/blob/main/scripts/make_appcast.sh) 生成 HTML 注释）并更新 `appcast.xml`。
...
- [ ] 按照 [macOS 发布](/platforms/mac/release) 获取确切命令和所需环境变量。
  ...
  - 如果进行公证，使用从 App Store Connect API 环境变量创建的 `yoyoclaw-notary` 钥匙串配置文件（参见 [macOS 发布](/platforms/mac/release)）。
...
- [ ] 验证注册表：`npm view yoyoclaw version`、`npm view yoyoclaw dist-tags` 和 `npx -y yoyoclaw@X.Y.Z --version`（或 `--help`）。

### 故障排除（来自 2.0.0-beta2 发布的笔记）

- **npm pack/publish 挂起或产生巨大 tarball**：`dist/YoyoClaw.app` 中的 macOS 应用包（和发布 zip）被扫入包中。通过 `package.json` 的 `files` 白名单发布内容来修复（包含 dist 子目录、docs、skills；排除应用包）。用 `npm pack --dry-run` 确认 `dist/YoyoClaw.app` 未列出。
...
- [ ] 为 `vX.Y.Z` 创建/刷新 GitHub 发布，**标题为 `yoyoclaw X.Y.Z`**（不仅仅是标签）；正文应包含该版本的**完整**变更日志部分（亮点 + 更改 + 修复），内联显示（无裸链接），且**不得在正文中重复标题**。
- [ ] 附加产物：`npm pack` tarball（可选）、`YoyoClaw-X.Y.Z.zip` 和 `YoyoClaw-X.Y.Z.dSYM.zip`（如果生成）。
- [ ] 提交更新后的 `appcast.xml` 并推送（Sparkle 从 main 获取源）。
- [ ] 从干净的临时目录（无 `package.json`），运行 `npx -y yoyoclaw@X.Y.Z send --help` 确认安装/CLI 入口点正常工作。
- [ ] 宣布/分享发布说明。

## 插件发布范围（npm）

我们只发布 `@yoyoclaw/*` 范围下的**现有 npm 插件**。不在 npm 上的内置插件保持**仅磁盘树**（仍在 `extensions/**` 中发布）。

获取列表的流程：

1. `npm search @yoyoclaw --json` 并捕获包名。
