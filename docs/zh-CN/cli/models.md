...
summary: "`yoyoclaw models` 的 CLI 参考（status/list/set/scan、别名、回退、认证）"
title: models
x-i18n:
...
`yoyoclaw models status` 显示已解析的默认模型/回退配置以及认证概览。
...
- 如果省略提供商，YoyoClaw 会将输入视为别名或**默认提供商**的模型（仅在模型 ID 不包含 `/` 时有效）。
...
`yoyoclaw plugins list` 查看已安装的提供商。
