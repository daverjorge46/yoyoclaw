...
summary: YoyoClaw Gateway 网关 CLI（`yoyoclaw gateway`）— 运行、查询和发现 Gateway 网关
title: gateway
x-i18n:
...
Gateway 网关是 YoyoClaw 的 WebSocket 服务器（渠道、节点、会话、hooks）。

本页中的子命令位于 `yoyoclaw gateway …` 下。
...
- 默认情况下，除非在 `~/.daverjorge46/yoyoclaw.json` 中设置了 `gateway.mode=local`，否则 Gateway 网关将拒绝启动。使用 `--allow-unconfigured` 进行临时/开发运行。
...
`gateway discover` 扫描 Gateway 网关信标（`_yoyoclaw-gw._tcp`）。

- 组播 DNS-SD：`local.`
- 单播 DNS-SD（广域 Bonjour）：选择一个域（示例：`yoyoclaw.internal.`）并设置分割 DNS + DNS 服务器；参见 [/gateway/bonjour](/gateway/bonjour)
