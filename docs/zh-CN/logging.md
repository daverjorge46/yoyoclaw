...
你可以在 `~/.daverjorge46/yoyoclaw.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/yoyoclaw.log"
  }
}
```
...
```bash
yoyoclaw channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.daverjorge46/yoyoclaw.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/daverjorge46/yoyoclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- YoyoClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `yoyoclaw plugins enable diagnostics-otel` 启用插件。
...
- `yoyoclaw.tokens`（计数器，属性：`yoyoclaw.token`、`yoyoclaw.channel`、`yoyoclaw.provider`、`yoyoclaw.model`）
- `yoyoclaw.cost.usd`（计数器，属性：`yoyoclaw.channel`、`yoyoclaw.provider`、`yoyoclaw.model`）
- `yoyoclaw.run.duration_ms`（直方图，属性：`yoyoclaw.channel`、`yoyoclaw.provider`、`yoyoclaw.model`）
- `yoyoclaw.context.tokens`（直方图，属性：`yoyoclaw.context`、`yoyoclaw.channel`、`yoyoclaw.provider`、`yoyoclaw.model`）

消息流：

- `yoyoclaw.webhook.received`（计数器，属性：`yoyoclaw.channel`、`yoyoclaw.webhook`）
- `yoyoclaw.webhook.error`（计数器，属性：`yoyoclaw.channel`、`yoyoclaw.webhook`）
- `yoyoclaw.webhook.duration_ms`（直方图，属性：`yoyoclaw.channel`、`yoyoclaw.webhook`）
- `yoyoclaw.message.queued`（计数器，属性：`yoyoclaw.channel`、`yoyoclaw.source`）
- `yoyoclaw.message.processed`（计数器，属性：`yoyoclaw.channel`、`yoyoclaw.outcome`）
- `yoyoclaw.message.duration_ms`（直方图，属性：`yoyoclaw.channel`、`yoyoclaw.outcome`）

队列 + 会话：

- `yoyoclaw.queue.lane.enqueue`（计数器，属性：`yoyoclaw.lane`）
- `yoyoclaw.queue.lane.dequeue`（计数器，属性：`yoyoclaw.lane`）
- `yoyoclaw.queue.depth`（直方图，属性：`yoyoclaw.lane` 或 `yoyoclaw.channel=heartbeat`）
- `yoyoclaw.queue.wait_ms`（直方图，属性：`yoyoclaw.lane`）
- `yoyoclaw.session.state`（计数器，属性：`yoyoclaw.state`、`yoyoclaw.reason`）
- `yoyoclaw.session.stuck`（计数器，属性：`yoyoclaw.state`）
- `yoyoclaw.session.stuck_age_ms`（直方图，属性：`yoyoclaw.state`）
- `yoyoclaw.run.attempt`（计数器，属性：`yoyoclaw.attempt`）

### 导出的 span（名称 + 关键属性）

- `yoyoclaw.model.usage`
  - `yoyoclaw.channel`、`yoyoclaw.provider`、`yoyoclaw.model`
  - `yoyoclaw.sessionKey`、`yoyoclaw.sessionId`
  - `yoyoclaw.tokens.*`（input/output/cache_read/cache_write/total）
- `yoyoclaw.webhook.processed`
  - `yoyoclaw.channel`、`yoyoclaw.webhook`、`yoyoclaw.chatId`
- `yoyoclaw.webhook.error`
  - `yoyoclaw.channel`、`yoyoclaw.webhook`、`yoyoclaw.chatId`、`yoyoclaw.error`
- `yoyoclaw.message.processed`
  - `yoyoclaw.channel`、`yoyoclaw.outcome`、`yoyoclaw.chatId`、`yoyoclaw.messageId`、`yoyoclaw.sessionKey`、`yoyoclaw.sessionId`、`yoyoclaw.reason`
- `yoyoclaw.session.stuck`
  - `yoyoclaw.state`、`yoyoclaw.ageMs`、`yoyoclaw.queueDepth`、`yoyoclaw.sessionKey`、`yoyoclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `yoyoclaw doctor`。
