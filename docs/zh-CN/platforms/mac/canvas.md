...
- `~/Library/Application Support/YoyoClaw/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `yoyoclaw-canvas://<session>/<path>`

示例：

- `yoyoclaw-canvas://main/` → `<canvasRoot>/main/index.html`
- `yoyoclaw-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `yoyoclaw-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
yoyoclaw nodes canvas present --node <id>
yoyoclaw nodes canvas navigate --node <id> --url "/"
yoyoclaw nodes canvas eval --node <id> --js "document.title"
yoyoclaw nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__yoyoclaw__/a2ui/
```
...
yoyoclaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
yoyoclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "yoyoclaw://agent?message=Review%20this%20design";
```
