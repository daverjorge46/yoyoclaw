---
read_when:
  - 在 DigitalOcean 上设置 YoyoClaw
  - 寻找便宜的 VPS 托管来运行 YoyoClaw
summary: 在 DigitalOcean 上运行 YoyoClaw（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 YoyoClaw

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 YoyoClaw Gateway 网关。
...
- `~/.yoyoclaw/` — 配置、凭证、会话数据
- `~/.yoyoclaw/workspace/` — 工作区（SOUL.md、记忆等）
