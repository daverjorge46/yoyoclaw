...
- 使用 macOS 应用的 **Remote over SSH** 模式（设置 → 通用 → "YoyoClaw runs"）。
...
- `yoyoclaw health` 和 `yoyoclaw status --deep` 现在通过 `ws://127.0.0.1:18789` 访问远程 Gateway 网关。
- `yoyoclaw gateway {status,health,send,agent,call}` 在需要时也可以通过 `--url` 指定转发的 URL。
