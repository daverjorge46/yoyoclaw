...
- 替代旧版 `gateway` 命令。CLI 入口点：`yoyoclaw gateway`。
...
- 配置热重载监视 `~/.daverjorge46/yoyoclaw.json`（或 `OPENCLAW_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）上启动 Canvas 文件服务器，从 `~/.yoyoclaw/workspace/canvas` 提供 `http://<gateway-host>:18793/__yoyoclaw__/canvas/`。使用 `canvasHost.enabled=false` 或 `OPENCLAW_SKIP_CANVAS_HOST=1` 禁用。
...
- macOS：`bot.molt.<profile>`（旧版 `com.yoyoclaw.*` 可能仍然存在）
- Linux：`yoyoclaw-gateway-<profile>.service`
- Windows：`YoyoClaw Gateway (<profile>)`

安装元数据嵌入在服务配置中：

- `OPENCLAW_SERVICE_MARKER=yoyoclaw`
...
- `OPENCLAW_STATE_DIR=~/.yoyoclaw-dev`
- `OPENCLAW_CONFIG_PATH=~/.yoyoclaw-dev/yoyoclaw.json`
...
- 当你在 `--dev` 下运行 `setup`/`onboard` 时，`agents.defaults.workspace` 默认变为 `~/.yoyoclaw/workspace-dev`。
...
```bash
yoyoclaw --profile main gateway install
yoyoclaw --profile rescue gateway install
```
...
- `health` — 完整健康快照（与 `yoyoclaw health --json` 形状相同）。
- `status` — 简短摘要。
...
- LaunchAgents 是按用户的，需要已登录的会话；对于无头设置，使用自定义 LaunchDaemon（未随附）。
  - `yoyoclaw gateway install` 写入 `~/Library/LaunchAgents/bot.molt.gateway.plist`
    （或 `bot.molt.<profile>.plist`；旧版 `com.yoyoclaw.*` 会被清理）。
  - `yoyoclaw doctor` 审计 LaunchAgent 配置，可以将其更新为当前默认值。
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 YoyoClaw 配置文件服务。
  我们仍然建议大多数设置**每台机器一个 Gateway 网关**；使用隔离的配置文件/端口进行冗余或救援机器人。参见[多个 Gateway 网关](/gateway/multiple-gateways)。
  - 清理：`yoyoclaw gateway uninstall`（当前服务）和 `yoyoclaw doctor`（旧版迁移）。
- `gateway install` 在已安装时是无操作的；使用 `yoyoclaw gateway install --force` 重新安装（配置文件/env/路径更改）。

捆绑的 mac 应用：

- YoyoClaw.app 可以捆绑基于 Node 的 Gateway 网关中继并安装标记为
  `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.yoyoclaw.*` 标签仍能干净卸载）的按用户 LaunchAgent。
- 要干净地停止它，使用 `yoyoclaw gateway stop`（或 `launchctl bootout gui/$UID/bot.molt.gateway`）。
- 要重启，使用 `yoyoclaw gateway restart`（或 `launchctl kickstart -k gui/$UID/bot.molt.gateway`）。
  - `launchctl` 仅在 LaunchAgent 已安装时有效；否则先使用 `yoyoclaw gateway install`。
  - 运行命名配置文件时，将标签替换为 `bot.molt.<profile>`。

## 监管（systemd 用户单元）

YoyoClaw 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们
建议单用户机器使用用户服务（更简单的 env，按用户配置）。
对于多用户或常驻服务器使用**系统服务**（无需 lingering，
共享监管）。

`yoyoclaw gateway install` 写入用户单元。`yoyoclaw doctor` 审计
单元并可以将其更新以匹配当前推荐的默认值。

创建 `~/.config/systemd/user/yoyoclaw-gateway[-<profile>].service`：

```
[Unit]
Description=YoyoClaw Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/yoyoclaw gateway --port 18789
Restart=always
RestartSec=5
...
创建 `/etc/systemd/system/yoyoclaw-gateway[-<profile>].service`（复制上面的单元，
...
- `yoyoclaw gateway health|status` — 通过 Gateway 网关 WS 请求 health/status。
- `yoyoclaw message send --target <num> --message "hi" [--media ...]` — 通过 Gateway 网关发送（对 WhatsApp 是幂等的）。
- `yoyoclaw agent --message "hi" --to <num>` — 运行智能体轮次（默认等待最终结果）。
- `yoyoclaw gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `yoyoclaw gateway stop|restart` — 停止/重启受监管的 Gateway 网关服务（launchd/systemd）。
- Gateway 网关辅助子命令假设 `--url` 上有运行中的 Gateway 网关；它们不再自动生成一个。

## 迁移指南

- 淘汰 `yoyoclaw gateway` 和旧版 TCP 控制端口的使用。
