---
read_when:
  - 你正在将 YoyoClaw 迁移到新的笔记本电脑/服务器
  - 你想保留会话、认证和渠道登录（WhatsApp 等）
summary: 将 YoyoClaw 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
本指南将 YoyoClaw Gateway 网关从一台机器迁移到另一台，**无需重新进行新手引导**。

迁移在概念上很简单：

- 复制**状态目录**（`$OPENCLAW_STATE_DIR`，默认：`~/.yoyoclaw/`）— 这包括配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.yoyoclaw/workspace/`）— 这包括你的智能体文件（记忆、提示等）。
...
- `--profile <name>`（通常变成 `~/.yoyoclaw-<profile>/`）
...
如果你有多个配置文件/状态目录（例如 `~/.yoyoclaw-main`、`~/.yoyoclaw-work`），分别归档每个。

### 步骤 1 — 在新机器上安装 YoyoClaw

在**新**机器上，安装 CLI（如果需要还有 Node）：

- 参见：[安装](/install)

在这个阶段，如果新手引导创建了一个新的 `~/.yoyoclaw/` 也没关系 — 你将在下一步覆盖它。
...
### 陷阱：只复制 `yoyoclaw.json`

`yoyoclaw.json` 是不够的。许多提供商在以下位置存储状态：
...
- `yoyoclaw status` 显示 Gateway 网关正在运行
...
- [YoyoClaw 在哪里存储数据？](/help/faq#where-does-yoyoclaw-store-its-data)
