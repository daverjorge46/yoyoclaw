...
**目标：** YoyoClaw Gateway 网关运行在 [Fly.io](https://fly.io) 机器上，具有持久存储、自动 HTTPS 和 Discord/渠道访问。
...
- 像对待密码一样对待这些 token。
- **优先使用环境变量而不是配置文件**来存储所有 API 密钥和 token。这可以避免密钥出现在 `yoyoclaw.json` 中，防止意外暴露或记录。
...
**注意：** 使用 `OPENCLAW_STATE_DIR=/data` 时，配置路径是 `/data/yoyoclaw.json`。
...
如果使用 `--allow-unconfigured`，Gateway 网关会创建最小配置。你在 `/data/yoyoclaw.json` 的自定义配置应该在重启时被读取。
