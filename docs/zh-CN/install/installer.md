...
YoyoClaw 提供两个安装器脚本（托管在 `openclaw.ai`）：
...
如果安装器完成但在新终端中找不到 `yoyoclaw`，通常是 Node/npm PATH 问题。参见：[安装](/install#nodejs--npm-path-sanity)。
...
- 如果是升级现有安装：运行 `yoyoclaw doctor --non-interactive`（尽力执行）。
- 对于 git 安装：安装/更新后运行 `yoyoclaw doctor --non-interactive`（尽力执行）。
...
如果你在**已有的 YoyoClaw 源码检出目录中**运行安装器（通过 `package.json` + `pnpm-workspace.yaml` 检测），它会提示：
...
此脚本将 `yoyoclaw` 安装到前缀目录（默认：`~/.yoyoclaw`），同时在该前缀下安装专用的 Node 运行时，因此可以在不想改动系统 Node/npm 的机器上使用。
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g yoyoclaw@latest`
  - `git`：克隆/构建源码检出并安装包装脚本
- 在升级和 git 安装时运行 `yoyoclaw doctor --non-interactive`（尽力执行）。
...
```powershell
iwr -useb https://openclaw.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\yoyoclaw"
```
