...
- `/usage cost` in chats: local cost summary aggregated from YoyoClaw session logs.
- CLI: `yoyoclaw status --usage` prints a full per-provider breakdown.
- CLI: `yoyoclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
