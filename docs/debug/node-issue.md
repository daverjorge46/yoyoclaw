...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in YoyoClaw
...
Running YoyoClaw via Node with `tsx` fails at startup with:

```
[yoyoclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `yoyoclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch yoyoclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node yoyoclaw.mjs status` works on Node 25.
