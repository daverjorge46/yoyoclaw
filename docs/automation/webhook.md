...
read_when:
  ...
  - Wiring external systems into YoyoClaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your YoyoClaw config directory (typically `~/.yoyoclaw/hooks/transforms`).
...
- `yoyoclaw webhooks gmail setup` writes `hooks.gmail` config for `yoyoclaw webhooks gmail run`.
