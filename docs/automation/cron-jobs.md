...
- Jobs persist under `~/.yoyoclaw/cron/` so restarts don’t lose schedules.
...
yoyoclaw cron list
yoyoclaw cron run <job-id>
yoyoclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.yoyoclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `yoyoclaw cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, YoyoClaw defaults to `announce`.
...
- Job store: `~/.yoyoclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.yoyoclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
yoyoclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
yoyoclaw cron edit <jobId> --agent ops
yoyoclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
yoyoclaw cron run <jobId>
yoyoclaw cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
yoyoclaw cron edit <jobId> \
...
```bash
yoyoclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`yoyoclaw system event`](/cli/system).
...
- YoyoClaw applies exponential retry backoff for recurring jobs after consecutive errors:
