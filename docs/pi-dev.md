...
This guide summarizes a sane workflow for working on the pi integration in YoyoClaw.
...
- Trigger the agent directly:
  - `pnpm yoyoclaw agent --message "Hello" --thinking low`
...
State lives under the YoyoClaw state directory. Default is `~/.yoyoclaw`. If `OPENCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `yoyoclaw.json` for config
