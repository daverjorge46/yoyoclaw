...
read_when:
  - You need detailed behavior for yoyoclaw onboard
...
This page is the full reference for `yoyoclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.daverjorge46/yoyoclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `yoyoclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `yoyoclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `yoyoclaw health`.
    - `yoyoclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.yoyoclaw/.env` so launchd can read it.
...
- OAuth credentials: `~/.yoyoclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.yoyoclaw/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.yoyoclaw/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`yoyoclaw onboard`](/cli/onboard)
