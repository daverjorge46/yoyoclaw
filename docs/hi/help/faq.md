...
4. आपको **macOS‑only टूल्स** के लिए ही Mac की आवश्यकता होती है। 7. iMessage के लिए, [BlueBubbles](/channels/bluebubbles) का उपयोग करें (अनुशंसित) - BlueBubbles सर्वर किसी भी Mac पर चलता है, और Gateway Linux या कहीं और चल सकता है। 8. यदि आप अन्य macOS‑only टूल्स चाहते हैं, तो Gateway को Mac पर चलाएँ या किसी macOS नोड के साथ पेयर करें।

5. Docs: [BlueBubbles](/channels/bluebubbles), [Nodes](/nodes), [Mac remote mode](/platforms/mac/remote)।
...
29. यदि आप फिर भी Bun के साथ प्रयोग करना चाहते हैं, तो इसे किसी non‑production Gateway पर करें
...
27. अपने बॉट से कहें "इस कार्य के लिए एक sub-agent स्पॉन करें" या `/subagents` का उपयोग करें।
28. अभी गेटवे क्या कर रहा है (और क्या वह व्यस्त है) यह देखने के लिए चैट में `/status` का उपयोग करें।

29. टोकन टिप: लंबे कार्य और sub-agents दोनों टोकन खर्च करते हैं। 31. यदि लागत चिंता का विषय है, तो
    `agents.defaults.subagents.model` के माध्यम से sub-agents के लिए एक सस्ता मॉडल सेट करें।

30. डॉक्युमेंट्स: [Sub-agents](/tools/subagents)।
...
45. ClawHub CLI इंस्टॉल करें (किसी एक पैकेज मैनेजर को चुनें):
...
13. नोड्स इनबाउंड प्रोवाइडर ट्रैफ़िक नहीं देखते; उन्हें केवल node RPC कॉल्स मिलती हैं।

14. यदि Gateway रिमोट होस्ट पर होस्टेड है तो मेरा एजेंट मेरे कंप्यूटर तक कैसे पहुँचे
...
18. Gateway को हमेशा-ऑन होस्ट (VPS/होम सर्वर) पर चलाएँ।

19. 20. Gateway होस्ट और अपने कंप्यूटर को एक ही tailnet पर रखें।
20. 21. सुनिश्चित करें कि Gateway WS पहुँचे योग्य है (tailnet bind या SSH टनल)।
21. 22. macOS ऐप को लोकली खोलें और **Remote over SSH** मोड (या डायरेक्ट tailnet) में कनेक्ट करें
        ताकि वह एक नोड के रूप में रजिस्टर हो सके।
22. 23. Gateway पर नोड को अप्रूव करें:
23. 24. अलग TCP ब्रिज की आवश्यकता नहीं है; नोड्स Gateway WebSocket के माध्यम से कनेक्ट होते हैं।
...
27. Tailscale कनेक्टेड है लेकिन मुझे कोई जवाब नहीं मिल रहा। अब क्या करूँ
...
41. Bot A, Bot B को एक संदेश भेजे, फिर Bot B सामान्य रूप से जवाब दे।
42. **CLI ब्रिज (जेनेरिक):** एक स्क्रिप्ट चलाएँ जो दूसरे Gateway को कॉल करे
    `openclaw agent --message ...` के साथ

43. `--deliver`, ऐसे चैट को टार्गेट करते हुए जहाँ दूसरा बॉट
...
44. openclaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
...
2. अलग-अलग VPSes का उपयोग केवल तब करें जब आपको hard isolation (security boundaries) या बहुत अलग configs की आवश्यकता हो जिन्हें आप साझा नहीं करना चाहते। 5. अन्यथा, एक Gateway रखें और
...
17. दूसरा Gateway केवल तब install करें जब आपको **hard isolation** या दो पूरी तरह अलग bots की आवश्यकता हो।

18. Docs: [Nodes](/nodes), [Nodes CLI](/cli/nodes), [Multiple gateways](/gateway/multiple-gateways)।
...
23. `gateway`, `discovery`, और `canvasHost` में बदलावों के लिए पूर्ण restart आवश्यक है।
...
1. 48. **अपने Mac पर install + login करें**
   - 49. Tailscale ऐप का उपयोग करें और उसी tailnet में sign in करें।

1. 50. **MagicDNS सक्षम करें (recommended)**
   - In the Tailscale admin console, enable MagicDNS so the VPS has a stable name.

1. **Use the tailnet hostname**
...
नहीं। 1. OpenClaw **आपके अपने खाते** पर चलता है, इसलिए अगर आप ग्रुप में हैं, तो OpenClaw उसे देख सकता है। 2. डिफ़ॉल्ट रूप से, ग्रुप रिप्लाई तब तक ब्लॉक रहते हैं जब तक आप सेंडर्स को अनुमति नहीं देते (`groupPolicy: "allowlist"`)।
...
33. ब्राउज़र एक्सेस शक्तिशाली है, लेकिन "इंसान जो कर सकता है वह सब" नहीं — एंटी-बॉट, CAPTCHAs, और MFA अभी भी ऑटोमेशन को ब्लॉक कर सकते हैं। 36. सबसे विश्वसनीय ब्राउज़र कंट्रोल के लिए, ब्राउज़र चलाने वाली मशीन पर Chrome extension relay का उपयोग करें (और Gateway कहीं भी रखें)।

34. बेस्ट-प्रैक्टिस सेटअप:
...
53. MiniMax M2.1 के अपने दस्तावेज़ हैं: [MiniMax](/providers/minimax) और
    [Local models](/gateway/local-models).

54. सामान्य नियम: उच्च-जोखिम वाले काम के लिए **सबसे अच्छा मॉडल जो आप वहन कर सकते हैं** उपयोग करें, और नियमित चैट या सारांश के लिए सस्ता
...
12. सुरक्षा नोट: छोटे या भारी रूप से क्वांटाइज़्ड मॉडल प्रॉम्प्ट
...
13. यदि आप फिर भी छोटे मॉडल चाहते हैं, तो सैंडबॉक्सिंग और सख्त टूल allowlists सक्षम करें।

14. दस्तावेज़: [Ollama](/providers/ollama), [Local models](/gateway/local-models),
...
13. 14. **2026.1.12** में अपग्रेड करें (या सोर्स `main` से चलाएँ), फिर गेटवे रीस्टार्ट करें।
14. 15. सुनिश्चित करें कि MiniMax कॉन्फ़िगर है (wizard या JSON), या env/auth प्रोफ़ाइल में MiniMax API key मौजूद है ताकि provider inject हो सके।
15. 16. सटीक मॉडल id (case-sensitive) उपयोग करें: `minimax/MiniMax-M2.1` या `minimax/MiniMax-M2.1-lightning`।
16. Run:
...
1. 37. क्या Gateway स्वस्थ है? `openclaw status`
1. 38. क्या UI के पास सही टोकन है? `openclaw dashboard`
1. 39. यदि रिमोट हैं, तो क्या टनल/Tailscale लिंक चालू है?
...
2. छोटे स्तर से शुरू करें। 5. केवल उन्हीं टूल्स और अकाउंट्स को एक्सेस दें जिनकी आपको वास्तव में ज़रूरत है, और ज़रूरत पड़ने पर
   बाद में विस्तार करें।

3. Docs: [Security](/gateway/security), [Pairing](/channels/pairing).
...
27. Pairing को इस प्रकार अनुमोदित करें:
...
42. Docs: [Thinking and verbose](/tools/thinking), [Security](/gateway/security#reasoning--verbose-output-in-groups).
