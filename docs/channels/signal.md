...
- YoyoClaw installed on your server (Linux flow below tested on Ubuntu 24).
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "YoyoClaw"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure YoyoClaw and restart the gateway.
5. Send a first DM and approve pairing (`yoyoclaw pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "YoyoClaw"` then scan the QR in Signal.
...
4. Configure YoyoClaw, restart gateway, verify channel:
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `yoyoclaw pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point YoyoClaw at it:
...
This skips auto-spawn and the startup wait inside YoyoClaw. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Approve via:
  - `yoyoclaw pairing list signal`
  - `yoyoclaw pairing approve signal <CODE>`
...
- **Typing indicators**: YoyoClaw sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, YoyoClaw forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `yoyoclaw doctor --fix`.
...
yoyoclaw pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/daverjorge46/yoyoclaw-$(date +%Y-%m-%d).log" | tail -20
```
