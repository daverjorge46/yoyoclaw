...
```bash
yoyoclaw plugins install @yoyoclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
yoyoclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "YoyoClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `yoyoclaw pairing list nextcloud-talk`
  - `yoyoclaw pairing approve nextcloud-talk <CODE>`
