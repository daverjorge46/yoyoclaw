...
Matrix is an open, decentralized messaging protocol. YoyoClaw connects as a Matrix **user**
...
```bash
yoyoclaw plugins install @yoyoclaw/matrix
```

Local checkout (when running from a git repo):

```bash
yoyoclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `yoyoclaw plugins install @yoyoclaw/matrix`
   - From a local checkout: `yoyoclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: YoyoClaw calls the same
     login endpoint, stores the access token in `~/.yoyoclaw/credentials/matrix/credentials.json`,
...
- On first connection, YoyoClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  YoyoClaw logs a warning.
...
`~/.yoyoclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `yoyoclaw pairing list matrix`
  - `yoyoclaw pairing approve matrix <CODE>`
...
- On startup, YoyoClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
