...
```bash
yoyoclaw pairing list discord
yoyoclaw pairing approve discord <CODE>
```
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in YoyoClaw config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces YoyoClaw auth and returns "not authorized".
...
```json5
{
  channels: {
    discord: {
      ...
      activityType: 1,
      activityUrl: "https://twitch.tv/yoyoclaw",
    },
  },
}
```
...
Discord voice messages show a waveform preview and require OGG/Opus audio plus metadata. YoyoClaw generates the waveform automatically, but it needs `ffmpeg` and `ffprobe` available on the gateway host to inspect and convert audio files.

Requirements and constraints:

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord does not allow text + voice message in the same payload).
- Any audio format is accepted; YoyoClaw converts to OGG/Opus when needed.
...
- If command deploy/state is stale, restart gateway and re-check with `yoyoclaw channels status --probe`.
