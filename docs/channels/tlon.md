...
Tlon is a decentralized messenger built on Urbit. YoyoClaw connects to your Urbit ship and can
...
```bash
yoyoclaw plugins install @yoyoclaw/tlon
```

Local checkout (when running from a git repo):

```bash
yoyoclaw plugins install ./extensions/tlon
```
...
By default, YoyoClaw blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `yoyoclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, YoyoClaw replies in-thread.
