...
read_when:
  - Setting up Twitch chat integration for YoyoClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. YoyoClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
yoyoclaw plugins install @yoyoclaw/twitch
```

Local checkout (when running from a git repo):

```bash
yoyoclaw plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "yoyoclaw", // Bot's Twitch account
