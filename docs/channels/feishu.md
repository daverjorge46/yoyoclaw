...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects YoyoClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
yoyoclaw plugins install @yoyoclaw/feishu
```

Local checkout (when running from a git repo):

```bash
yoyoclaw plugins install ./extensions/feishu
```
...
If you just installed YoyoClaw, run the wizard:
...
2. Configuring app credentials in YoyoClaw
...
1. You already ran `yoyoclaw channels add` for Feishu
2. The gateway is running (`yoyoclaw gateway status`)
...
Edit `~/.daverjorge46/yoyoclaw.json`:
...
```bash
yoyoclaw pairing approve feishu <CODE>
```
...
2. Run `yoyoclaw logs --follow` and look for `chat_id`
...
2. Run `yoyoclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `yoyoclaw gateway status`  | Show gateway status           |
| `yoyoclaw gateway install` | Install/start gateway service |
| `yoyoclaw gateway stop`    | Stop gateway service          |
| `yoyoclaw gateway restart` | Restart gateway service       |
| `yoyoclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `yoyoclaw logs --follow`
...
5. Ensure the gateway is running: `yoyoclaw gateway status`
6. Check logs: `yoyoclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.yoyoclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.yoyoclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
