...
This document explains how YoyoClaw manages sessions end-to-end:
...
YoyoClaw is designed around a single **Gateway process** that owns session state.
...
YoyoClaw persists sessions in two layers:
...
- Transcripts: `~/.yoyoclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

YoyoClaw resolves these via `src/config/sessions.ts`.
...
YoyoClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (YoyoClaw consumes the events, but Pi decides when to compact).
...
YoyoClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, YoyoClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, YoyoClaw leaves it alone.
...
- `yoyoclaw status` (CLI)
- `yoyoclaw sessions` / `sessions --json`
...
YoyoClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- YoyoClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, YoyoClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
YoyoClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but YoyoClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `yoyoclaw status`.
