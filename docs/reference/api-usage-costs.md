...
YoyoClaw features that can generate provider usage or paid API calls.
...
- `yoyoclaw status --usage` and `yoyoclaw channels list` show provider **usage windows**
...
YoyoClaw can pick up credentials from:
...
- `yoyoclaw status --usage`
- `yoyoclaw models status --json`
...
`yoyoclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
