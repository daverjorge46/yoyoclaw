...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node yoyoclaw.mjs daemon install --force --runtime node"
      log "  node yoyoclaw.mjs daemon restart"
      log ""
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node yoyoclaw.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node yoyoclaw.mjs daemon restart"
