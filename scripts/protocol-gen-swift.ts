...
const outDirs = [
  path.join(repoRoot, "apps", "macos", "Sources", "OpenClawProtocol"),
  path.join(repoRoot, "apps", "shared", "OpenClawKit", "Sources", "OpenClawProtocol"),
];

// Split files when they exceed this line count (leave room for header + footer)
...
async function generate() {
  ...
  const structs: Array<{ name: string; content: string; lines: number }> = [];
  for (const [name, schema] of definitions) {
    if (name === "GatewayFrame") {
      continue;
    }
    if (schema.type === "object") {
      const content = emitStruct(name, schema);
      const lines = content.split("\n").length;
      structs.push({ name, content, lines });
    }
  }

  // Split structs into multiple files based on line count
  const files: Array<{ name: string; content: string[] }> = [
    { name: "GatewayModels.swift", content: [header] },
  ];
  let currentFileIndex = 0;
  let currentLines = header.split("\n").length;

  for (const struct of structs) {
    // Check if adding this struct would exceed the limit
    if (currentLines + struct.lines > MAX_LINES_PER_FILE && currentFileIndex === 0) {
      // Start a new extension file
      currentFileIndex++;
      files.push({
        name: `GatewayModels+Part${currentFileIndex}.swift`,
        content: [
          "// Generated by scripts/protocol-gen-swift.ts — do not edit by hand\nimport Foundation\n",
        ],
      });
      currentLines = 3; // header lines
    } else if (currentLines + struct.lines > MAX_LINES_PER_FILE && currentFileIndex > 0) {
      // Start another extension file
      currentFileIndex++;
      files.push({
        name: `GatewayModels+Part${currentFileIndex}.swift`,
        content: [
    ...
    files[currentFileIndex].content.push(struct.content);
    currentLines += struct.lines;
  }

  // Add GatewayFrame enum to the main file (it depends on RequestFrame, ResponseFrame, EventFrame)
  files[0].content.push(emitGatewayFrame());

  // Write all files to both output directories
