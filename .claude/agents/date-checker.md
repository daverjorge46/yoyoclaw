...
2. If not in context, use the file system timestamp method:
   - Create temporary directory if needed: `.yoyo-dev/specs/`
   - Create temporary file: `.yoyo-dev/specs/.date-check`
...
mkdir -p .yoyo-dev/specs/

# Create temporary file
touch .yoyo-dev/specs/.date-check

# Read file with ls -la to see timestamp
ls -la .yoyo-dev/specs/.date-check

# Extract date from the timestamp
# Parse the date to YYYY-MM-DD format

# Clean up
rm .yoyo-dev/specs/.date-check
```
