...
const PROJECTS = {
  bita: {
    name: '幣塔',
    groups: [
      '-5148508655', '-5159438640', '-5030731997', '-5070604096',
      '-5186655303', '-5295280162', '-5023713246', '-5297227033', '-1003849990504'
    ]
  },
  xo: {
...
function extractTopics(projectId, days = 7) {
  ...
  try {
    ...
    // 簡單詞頻統計
    const wordCount = {};
    const stopWords = new Set(['的', '是', '在', '了', '和', '有', '我', '你', '他', '她', '它', '這', '那', '就', '都', '也', '還', '不', '會', '可以', '什麼', '怎麼', '為什麼', '嗎', '呢', '吧', '啊', '哦', '嗯']);
    ...
    console.log(`   Top topics: ${topics.slice(0, 5).map(t => t.word).join(', ')}`);
...
`;
  ...
  for (const [projectId, project] of Object.entries(PROJECTS)) {
    ...
    if (fs.existsSync(topicsFile)) {
      ...
      report += `**熱門主題**: ${topics.topics.slice(0, 5).map(t => t.word).join(', ')}\n\n`;
    } else {
      report += `_尚無數據_\n\n`;
    }
  }
