# flipflop-travel 升級規格書

**建立時間**：2026-02-05 11:43 UTC  
**最後更新**：2026-02-05 12:08 UTC  
**需求來源**：Mimi（姐姐）  
**審批狀態**：待 Cruz 批准  
**預估工時**：4-5 小時（含全部需求）
...
3. 按「套用到全部團員」
4. 全部人自動帶入
5. 特殊的（小孩、不同方案）再個別點進去改

**價值**：20 人的團從「點 20 次、填 20 次」→「填 1 次、點 1 下」
...
- categories 陣列新增：
  ...
  - `{ name: '票券', emoji: '🎫', amount, status }`
- 扣款順序建議：機票 > 簽證 > 住宿 > 包車 > 票券 > 網卡
...
- 6 個費用輸入框
- 「套用到全部團員」按鈕
- 呼叫 `api/members/batch_update`

### 部署流程

1. **DB Migration**：執行 ALTER TABLE
2. **程式碼修改**：前後端改動
3. **Git Push**：觸發 Vercel 自動部署
4. **驗收測試**：
   - 建測試團
   - 批次套用預設費用
   - 確認數字正確
...
- [ ] DB schema 有 `visa_amount`, `voucher_amount` 欄位
- [ ] Admin 編輯團員：可填簽證/票券金額，總額正確
- [ ] Admin 批次套用：一鍵更新全部團員費用
- [ ] 團員端 my_status：categories 正確顯示新項目
- [ ] 不影響現有資料（已繳金額、狀態不被覆蓋）
...
- [ ] Vercel 專案權限（Cruz 會提供）
- [ ] 扣款優先順序確認（簽證排在哪？）
- [ ] 是否需要「選擇性套用」（例如只套用住宿，不套用機票）
- [ ] 已結束的團要排最後還是完全隱藏？
- [ ] 行程分享：用「複製連結」還是「LIFF 分享」？
...
- [ ] `api/members/batch_update.js` - **新增**批次更新 API
- [ ] `api/trips/list.js` - 按 start_date 排序
...
- [ ] `TripManagePage.jsx` - 加「分享行程」按鈕
- [ ] `BatchFeeForm.jsx` - **新增**批次費用表單元件
