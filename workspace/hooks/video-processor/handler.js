...
async function handler(event) {
  ...
  const from = ctx.senderName || ctx.senderId;

  // Only process Telegram messages with video
  if (channel !== "telegram") return;
  if (mediaType !== "video") return;

  // rawEvent may contain file size info
  let sizeMB = 0;
  try {
    const raw = ctx.rawEvent ? JSON.parse(ctx.rawEvent) : {};
    sizeMB = (raw.fileSize || 0) / (1024 * 1024);
  } catch {
    // no file size available
  }
  if (sizeMB > 0 && sizeMB <= MAX_SIZE_MB) return; // Small enough, let Moltbot handle it

  console.log(
    `[video-processor] Large video detected: ${sizeMB.toFixed(2)}MB from ${from} in ${chatId}`,
  );
  ...
  try {
    ...
    // 6. Push a message so the agent knows about the processed video
    event.messages.push(
      `[影片已處理: ${duration}秒, ${resolution}, ${frameCount}張關鍵幀已提取到 output/]`,
    );
